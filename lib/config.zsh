...
_gwt_validate_config() {
  if [[ -n "$GWT_PROVIDER" ]] && [[ -z "${GWT_PROVIDERS[$GWT_PROVIDER]:-}" ]]; then
    ...
    echo "gwt: available providers: ${(k)GWT_PROVIDERS}" >&2
    return 1
  fi
  return 0
}
...
# Show current config
_gwt_show_config() {
  echo "\033[1mGWT Configuration\033[0m\n"
  
  # Show active config file
  local config_file found=false
  for config_file in "${GWT_CONFIG_PATHS[@]}"; do
    if [[ -f "$config_file" ]]; then
      echo "Config file: \033[32m$config_file\033[0m"
      found=true
      break
    fi
  done
  $found || echo "Config file: \033[33m(none)\033[0m"
