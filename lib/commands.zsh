...
_gwt_cmd_list() {
  ...
  while IFS= read -r line; do
    if [[ "$line" == worktree* ]]; then
      ...
      local resolved_wt=$(_gwt_realpath "$wt_path")
...
_gwt_cmd_clean() {
  ...
  while IFS= read -r line; do
    if [[ "$line" == worktree* ]]; then
      ...
      read -r _
      read -r branch_line
      wt_branch=$([[ "$branch_line" == branch* ]] && echo "${branch_line#branch refs/heads/}" || echo "")
  ...
  case "$1" in
    ...
    provider|providers)
      if [[ -n "$2" ]]; then
        ...
        echo "\033[1mCurrent provider:\033[0m $GWT_PROVIDER\n"
        _gwt_list_providers
        echo ""
